...
//https://github.com/rocaltair/skiplist
/*
local skiplist = require("skiplist")

local sl = skiplist.new(function(a, b, scoreA, scoreB, pdiff)
...
static slNode_t* getNode(lua_State* L, int sl_idx, int node_idx) {
  sl_t* sl;
  ...
  luaL_argcheck(L, !lua_isnoneornil(L, node_idx), node_idx, "number|string|table|udata required!");
  lua_getuservalue(L, sl_idx);
  lua_getfield(L, -1, "node_map");
  lua_pushvalue(L, node_idx);
  lua_rawget(L, -2);
  if (lua_isuserdata(L, -1)) {
    slNode_t* p = (slNode_t*)lua_touserdata(L, -1);
    lua_settop(L, top);
    return p;
  }
  lua_settop(L, top);
...
static int compInLua(slNode_t* nodeA, slNode_t* nodeB, sl_t* sl, void* ctx) {
  ...
  if (!lua_isfunction(L, -1)) {
    ...
    return luaL_error(L, "function not found! type=%s, value=%s,top=%d", tname, lua_tostring(L, -1), top);
  }
#endif
...
static int luac_skiplist_gc(lua_State* L) {
  sl_t* sl = CHECK_SL(L, 1);
  slDestroy(sl, NULL, NULL);
  return 0;
}

static int luac_insert(lua_State* L) {
  double score;
  sl_t* sl;
  slNode_t* node;
  int level;
  ...
  if ((node = slCreateNode(level, NULL, score)) == NULL)
    return luaL_error(L, "no memory in luac_insert");
...
static int luac_update(lua_State* L) {
  int level;
  double score;
  int cur = 0;
  sl_t* sl = CHECK_SL(L, 1);
  slNode_t* node = getNode(L, 1, 2);
  ...
  if (node != NULL) {
    ...
    level = slRandomLevel();
    if ((node = slCreateNode(level, NULL, score)) == NULL) {
      return luaL_error(L, "no memory in luac_update");
    }
    node->udata = node;
    lua_getuservalue(L, 1);
    lua_getfield(L, -1, "node_map");
...
#if ENABLE_LSL_INDEX
static int luac_index(lua_State* L) {
  slNode_t* node;
  sl_t* sl = CHECK_SL(L, 1);
  (void)sl;
...
static int luac_exists(lua_State* L) {
  sl_t* sl = CHECK_SL(L, 1);
  slNode_t* node = getNode(L, 1, 2);
  (void)sl;
...
static int luac_rank_iterator(lua_State* L) {
  slNode_t* node;
  slNode_t* next;
...
static int luac_rank_pairs(lua_State* L) {
  slNode_t* node;
  sl_t* sl = CHECK_SL(L, 1);
  int rankMin = (int)luaL_optinteger(L, 2, 1);
  int rankMax = (int)luaL_optinteger(L, 3, sl->size);
  ...
  lua_pushlightuserdata(L, (void*)node);
  lua_pushcclosure(L, luac_rank_iterator, 4);
  return 1;
}

static int luac_get_by_rank(lua_State* L) {
  slNode_t* node;
  sl_t* sl = CHECK_SL(L, 1);
...
static int luac_del_by_rank(lua_State* L) {
  int cur = 0;
  int ret;
  slNode_t* node;
  double score;
...
static int luac_get_score(lua_State* L) {
  sl_t* sl = CHECK_SL(L, 1);
  slNode_t* node = getNode(L, 1, 2);
  (void)sl;
  if (node == NULL)
...
static int luac_score_range(lua_State* L) {
  int i;
  ...
  double dmax = luaL_optnumber(L, 3, DBL_MAX);

  luaL_argcheck(L, dmin < dmax, 3, "max should greater or equal than min");
  pMin = slFirstGEThan(sl, dmin);
  pMax = slLastLEThan(sl, dmax);
...
static int luac_rank_of(lua_State* L) {
  int rank = 0;
  sl_t* sl = CHECK_SL(L, 1);
  int cur = 0;
  slNode_t* node = getNode(L, 1, 2);
...
static int luac_rank_range(lua_State* L) {
  int n;
  sl_t* sl = CHECK_SL(L, 1);
  slNode_t* node;
  int rankMin = (int)luaL_optinteger(L, 2, 1);
...
static int luac_next(lua_State* L) {
  slNode_t* next = NULL;
  sl_t* sl = CHECK_SL(L, 1);
  slNode_t* node = getNode(L, 1, 2);
  (void)sl;
...
static int luac_prev(lua_State* L) {
  slNode_t* prev = NULL;
  sl_t* sl = CHECK_SL(L, 1);
  slNode_t* node = getNode(L, 1, 2);
  (void)sl;
...
static void deleteCb(void* udata, void* ctx) {
  lua_State* L = (lua_State*)ctx;
  int top = lua_gettop(L);
  /* node->udata = node, see luac_insert */
...
static int luac_del_rank_range(lua_State* L) {
  sl_t* sl = CHECK_SL(L, 1);
  int imin = (int)luaL_checkinteger(L, 2);
  int imax = (int)luaL_optinteger(L, 3, imin);
  int n;
  luaL_argcheck(L, 1 <= imin && imin <= imax && imin <= sl->size, 2, "min [1, size]");
  luaL_argcheck(L, 1 <= imax && imin <= imax && imax <= sl->size, 3, "max [1, size]");
  lua_settop(L, 3);
  lua_getuservalue(L, 1);
  lua_getfield(L, -1, "value_map");		/*idx = 5*/
  lua_getfield(L, -2, "node_map");		/*idx = 6*/
  n = slDeleteByRankRange(sl, imin, imax, deleteCb, L);
  lua_pushinteger(L, n);
  return 1;
}
...
static void init_metatable(lua_State* L) {
  luaL_Reg lmethods[] = {
    ...
    { "delete",            luac_delete         },
    { "exists",            luac_exists         },
    { "get_by_rank",       luac_get_by_rank    },
    { "del_by_rank",       luac_del_by_rank    },
    { "del_by_rank_range", luac_del_rank_range },
    { "rank_of",           luac_rank_of        },
    { "rank_range",        luac_rank_range     },
    { "get_score",         luac_get_score      },
    { "score_range",       luac_score_range    },
    { "next",              luac_next           },
    { "prev",              luac_prev           },
    { "size",              luac_size           },
    { "rank_pairs",        luac_rank_pairs     },
    { NULL,                NULL                },
  };
  luaL_newmetatable(L, CLASS_SKIPLIST);
  luaL_newlib(L, lmethods);
#if ENABLE_LSL_INDEX
  lua_pushcclosure(L, luac_index, 1);
#endif
  lua_setfield(L, -2, "__index");
  lua_pushcfunction(L, luac_update);
  lua_setfield(L, -2, "__newindex");
  lua_pushcfunction(L, luac_skiplist_gc);
  lua_setfield(L, -2, "__gc");
  lua_pushcfunction(L, luac_size);
